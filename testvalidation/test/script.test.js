...
describe('💡 실습을 잘 따라했는지 확인합니다', () => {
  ...
  it('아이디 입력창 `#username`에 네 글자 이하를 입력하면, 실패 메시지가 보여야 합니다', () => {
    ...
    expect(
      document.querySelector('.failure-message').classList.contains('hide')
    ).to.be.false;
    expect(
      document.querySelector('.success-message').classList.contains('hide')
    ).to.be.true;
  });
...
describe('💡 Bare Minimum Requirements - HTML을 이용해 목업을 만드세요', () => {
  ...
  it('처음 페이지를 띄울 때 시각적 피드백은 화면에 보이지 않습니다', () => {
    expect(
      document.querySelector('.mismatch-message').classList.contains('hide')
    ).to.be.true;
  });
});
